...
void RespondToDetection(tflite::ErrorReporter* error_reporter,
  ...
  client.publish("M5Atom", score_str);  //Publishes a message to the specified topic.
  ...
  if (person_score > 200) {
    if (((esp_timer_get_time() - elapsed_time)/1000) >= HOLD_TIME) {
      ...
      // KBH esp_err_t esp_ret = smtp_client_send_email(jpeg_image, jpeg_img_size);
      // KBH if (esp_ret != ESP_OK) {
      // KBH   ESP_LOGE(TAG, "Failed to send the email, returned %02X", esp_ret);
      // KBH }
      elapsed_time = 0;
    }
  }
